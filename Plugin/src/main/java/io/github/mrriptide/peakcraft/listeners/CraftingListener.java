        ...
        if (recipe instanceof ShapedRecipe){
            ...
            for (int r = 0; r < shapedRecipe.getShape().length; r++) {
                for (int c = 0; r < shapedRecipe.getShape()[0].length(); c++) {
                    ingredients[valid_rows.get(r) * 3 + valid_cols.get(c)].setAmount(ingredients[valid_rows.get(r) * 3 + valid_cols.get(c)].getAmount() - shapedRecipe.getIngredientMap().get(shapedRecipe.getShape()[r].charAt(c)).getCount());
                }
            }
